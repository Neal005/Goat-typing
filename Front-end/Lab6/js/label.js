...
let btn_lang=document.querySelectorAll(".multi-lang");
let translate=document.querySelectorAll(".lang");
//Chưa click (
if(!id){
	localStorage.setItem("lang","vi-VN");
	id="vi-VN";
}
Array.from(translate).forEach((item)=>{
  let clss=item.getAttribute("id");
  console.log(clss);
  item.innerHTML=labels[clss][id];
})
//)
//Đã click
Array.from(btn_lang).forEach((btn)=>{
  btn.addEventListener('click', (el)=>{
    console.log(el.target.getAttribute("data-lang"));
